...
def write_markdown(path: Path, rows: list[dict], missing: list[str]) -> None:
    lines = [
        "# SAM2.1-L two-clock validation comparison",
        "",
        "Phase-balanced SA-V val. Latency is the matched single-stream mean; "
        "refresh rate is measured from actual encoder calls.",
        "",
        "| Model | R | Skip | J&F | J | F | Refresh % | Model ms | Rank P95 max ms | Wall ms/frame | ΔJ&F vs O0 R1 | ΔJ&F vs official matched | Model speedup | Wall speedup |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            "| {model} | R{refresh_interval} | {skip_frames} | {jf} | {j} | {f} | {refresh} | {model_ms} | {p95} | {wall_ms} | {delta_o0} | {delta_matched} | {model_speedup}x | {wall_speedup}x |".format(
                **row,
                jf=_fmt(row["J&F"], 1),
                j=_fmt(row["J"], 1),
                f=_fmt(row["F"], 1),
...
def main() -> None:
    ...
    write_markdown(args.out_dir / "comparison.md", rows, missing)
