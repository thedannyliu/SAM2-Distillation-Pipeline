...
| `Q1_tinyvit_overfit16_t8_500ep` | TinyViT-21M A02 image/mask path plus coherently initialized official temporal path | Fixed deterministic 16-video subset, T8, 500 epochs (about 2k optimizer steps); task + memory + propagated-logit + pointer losses | Failure to drive losses down on 16 videos means the TinyViT/temporal interface or supervision is broken. Successful overfit but weak full val/test indicates generalization/data-recipe failure. |
...
- For Q1, inspect raw and EMA W&B losses. Require a clear reduction in task,
  mask-logit, and memory losses; full val/test are diagnostic and are not used
  as an overfit success criterion.
